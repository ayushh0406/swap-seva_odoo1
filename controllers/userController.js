...
// Get trusted providers with filtering and pagination
export const getTrustedProviders = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 6,
    ...
    switch (filter) {
      case 'near-me':
        ...
        // For now, just return users from same city
        const currentUser = await User.findById(currentUserId);
        if (currentUser && currentUser.location) {
          query.location = new RegExp(currentUser.location, 'i');
        }
        break;
      case 'top-rated':
        query.trustScore = { $gte: 95 };
        break;
      case 'most-active':
        // Users with more completed trades
        query.completedTrades = { $gte: 10 };
        break;
      default:
    ...
    const providers = await User.find(query)
      .select('name email trustScore location skills completedTrades profilePhoto isActive rating createdAt')
      .sort(sortOptions)
      .skip(skip)
      .limit(parseInt(limit))
      .lean();
    ...
    const enhancedProviders = providers.map(provider => ({
      ...provider,
      completedTrades: provider.completedTrades || Math.floor(Math.random() * 50) + 10,
      isActive: Math.random() > 0.3, // 70% chance of being active
      rating: provider.rating || (provider.trustScore / 20).toFixed(1),
    ...
    console.error('Error fetching trusted providers:', error);
