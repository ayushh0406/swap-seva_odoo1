...
export const signup = async (req, res) => {
  try {
    ...
    const userData = {
      ...
      // Location is now a simple string field
      location: city && city.trim() ? city.trim() : undefined
    };

    // Handle skills - store as skills array of strings
    if (skills && Array.isArray(skills)) {
      userData.skills = skills.map(skill => String(skill).trim()).filter(Boolean);
    }
