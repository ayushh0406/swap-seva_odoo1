...
export const findMatches = async (req, res) => {
  try {
    ...
    // Simple location-based filtering for users with similar location strings
    let finalMatches = potentialMatches;
    if (user.location && user.location.trim()) {
      // Filter for users in the same city/location
      const sameLocationUsers = potentialMatches.filter(match => 
        match.location && 
        match.location.toLowerCase().includes(user.location.toLowerCase()) ||
        user.location.toLowerCase().includes(match.location.toLowerCase())
      );
      
      // If we have same location users, prioritize them, otherwise use all matches
      if (sameLocationUsers.length > 0) {
        finalMatches = sameLocationUsers;
      }
    }

    // Calculate match score (basic implementation)
    const matchesWithScores = finalMatches.map(match => {
      let score = 0;
